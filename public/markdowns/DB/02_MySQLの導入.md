...
ここでは「testuser」というユーザーを新たに作成してみます。
...
このコマンドでは「testuser」というユーザーを、パスワード「test」で作成しています。
@の後ろは、接続可能なホスト名を表しており、localhostと指定すると、自身のサーバーからのみ接続可能となります。
なお、SQLでは大文字と小文字は区別されませんが、パスワードの「test」の部分（'で囲われた部分）は大文字と小文字を区別するので注意して下さい。
...
その後、先ほど作成したtestuserを指定して接続をします。
testuserではパスワードを設定したので、-pのオプションを指定します。

```bash
> mysql -u testuser -p
```

パスワードの入力を求められるので、「test」を入力します。
※画面上パスワードは表示されませんが、入力はされています。
接続に成功すればユーザーの作成はうまくいっています。
これから先の内容は「testuser」でmysqlへ接続するようにしてください。
...
左のツリーに「testdb」が表示され、選択することでデータベースに対する操作が可能になります。
データベース名は任意ですが、このテキストでは「testdb」として話を進めます。
